...
f_start = 0.30   # 0.1 GHz = 100 MHz
f_end = 2.30   # 6.5 GHz
f_step = 0.01   # 0.1 GHz = 100 MHz


def measure_4pow():
    src.write('APPLY 3.0v,50ma,1')
    ...
    fs = [round(x, 2) for x in np.linspace(f_start, f_end, int((f_end - f_start) / f_step) + 1, endpoint=True)]

    for f_gen in fs:
        print('set freq', f_gen)

        gen.write(f'SOUR:FREQ:CW {f_gen}ghz')
        gen.write(':POW:POW 0.0dbm')
        gen.write('OUTP ON')
    ...
    df.to_excel(f'xlsx/PP1U-0dBm-3_0V-{datetime.datetime.now().isoformat().replace(":", ".")}.xlsx')
